    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            if (password !== confirm_password||(!username)) {
                location.assign('${pageContext.request.contextPath}/signup');
                email.value = '';
                password.value = '';
            } else {
                const res = await fetch('${pageContext.request.contextPath}/signup', { method: 'POST', body: JSON.stringify({ username, password , email}), headers: { 'Content-Type': 'application/json' } })
                if(res.status===200)
